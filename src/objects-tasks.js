...
function removeProperties(obj, properties) {
  const newObj = obj;
  properties.forEach((e) => delete newObj[e]);
  return newObj;
}
...
function makeWord(obj) {
  ...
  const lettersLength = Math.max(...Object.values(obj).flat()) + 1;
  const letters = Array(lettersLength);
...
function sortCitiesArray(arr) {
  const newArr = arr.sort((a, b) => {
    const str1 = a.country + a.city;
    const str2 = b.country + b.city;
